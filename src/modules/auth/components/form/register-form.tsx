...
import { useLoginForm } from "../../hooks/useLoginForm"; // Manteniendo el hook de login
import { FieldInfo } from "@/modules/core/components/input/form-info";

export function RegisterForm() {
  const { form, loginUserMutation } = useLoginForm(); // Manteniendo el hook de login
    ...
    <div className="min-h-screen flex items-center justify-end">
      <div className="min-h-screen bg-white p-8 rounded-xl shadow-2xl w-auto h-auto lg:w-[450px]">
        ...
        <h1 className="text-3xl text-center uppercase font-bold tracking-[5px] text-black mb-8">
          Crear <span className="text-emerald-700">cuenta</span>
        </h1>
        <form
          onSubmit={(e) => {
            e.preventDefault();
        ...
        <div className="flex flex-col items-center gap-4">
          <span className="flex items-center gap-2">
            ¿Ya tienes cuenta?{" "}
            <a href="/auth/login" className="text-primary hover:text-gray-500 transition-colors font-bold">
              Inicia sesión
            </a>
          </span>
        </div>
